...
-- [[ VERSION: V25 STABLE (NO ERRORS) | AUTHOR: GHOST66266 ]] --
...
local function Tween(obj, props, time, style, dir)
	if not obj then return end -- Sécurité anti-crash "nil value"
...
local function AddClickEffect(Btn)
	...
	-- Correction position automatique
...
function Library:Welcome(TitleText, SubText)
	...
	-- Note: J'ai supprimé toute référence à TextSpacing ici.

	-- ANIMATION BLOQUANTE (Pas de task.spawn)
...
function Library:CreateWindow(Config)
	...
	Instance.new("UICorner", Main).CornerRadius = UDim.new(0, 6); Instance.new("UIStroke", Main).Color = Library.Theme.Outline
	...
	function WindowFuncs:AddTab(Name)
		local TabBtn = Instance.new("TextButton", TabContainer); TabBtn.Size = UDim2.new(1,0,0,36); TabBtn.BackgroundTransparency = 1; TabBtn.Text = ""
		...
		TabLabel.TextSize = Library.Theme.TextSize.Tab -- [DYNAMIC]
		...
		function TabFuncs:AddSection(Title)
			...
			SecTitle.TextSize = Library.Theme.TextSize.Section -- [DYNAMIC]
			...
			local List = Instance.new("UIListLayout", Container); List.Padding = UDim.new(0, 6); List:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() Container.Size = UDim2.new(1,-20,0,List.AbsoluteContentSize.Y); Section.Size = UDim2.new(1,0,0,List.AbsoluteContentSize.Y+40) end)

			local SecFuncs = {}
			function SecFuncs:AddButton(Text, Callback)
				local Btn = Instance.new("TextButton", Container); Btn.Size = UDim2.new(1,0,0,32); Btn.BackgroundColor3 = Library.Theme.Main; Btn.Text = Text; Btn.TextColor3 = Library.Theme.Text; Btn.Font = Enum.Font.GothamBold
				Btn.TextSize = Library.Theme.TextSize.Element -- [DYNAMIC]
				Btn.AutoButtonColor = false; Instance.new("UICorner", Btn).CornerRadius = UDim.new(0, 4); Instance.new("UIStroke", Btn).Color = Library.Theme.Outline
			...
			function SecFuncs:AddToggle(Text, Default, Callback)
				local Tgl = Instance.new("TextButton", Container); Tgl.Size = UDim2.new(1,0,0,32); Tgl.BackgroundTransparency = 1; Tgl.Text = ""; AddClickEffect(Tgl)
				...
				Lab.TextSize = Library.Theme.TextSize.Element -- [DYNAMIC]
			...
			function SecFuncs:AddDropdown(Text, Items, Default, Callback)
				...
				local Btn = Instance.new("TextButton", Drop); Btn.Size = UDim2.new(1,0,0,32); Btn.BackgroundColor3 = Library.Theme.Main; Btn.Text = ""; Instance.new("UICorner", Btn).CornerRadius = UDim.new(0, 4); Instance.new("UIStroke", Btn).Color = Library.Theme.Outline; AddClickEffect(Btn)
				...
				local Open = false; Btn.MouseButton1Click:Connect(function() Open = not Open; Tween(Drop, {Size = UDim2.new(1,0,0, Open and (math.min(#Items, 5)*27+40) or 32)}); Tween(Arrow, {Rotation = Open and 180 or 0}); Tween(List, {Size = UDim2.new(1,0,0, Open and (math.min(#Items, 5)*27) or 0)}) end)
				for _, item in pairs(Items) do local IB = Instance.new("TextButton", List); IB.Size = UDim2.new(1,0,0,25); IB.BackgroundColor3 = Library.Theme.Main; IB.Text = "  " .. item; IB.TextColor3 = Library.Theme.TextDark; IB.Font = Enum.Font.Gotham; IB.TextSize = Library.Theme.TextSize.Element; IB.TextXAlignment = Enum.TextXAlignment.Left; IB.MouseButton1Click:Connect(function() Open = false; Lab.Text = Text..": "..item; Tween(Drop, {Size = UDim2.new(1,0,0,32)}); Tween(Arrow, {Rotation = 0}); pcall(Callback, item) end) end; List.CanvasSize = UDim2.new(0,0,0,#Items*27)
			end
			function SecFuncs:AddTextbox(Text, Callback)
				local BoxFrame = Instance.new("Frame", Container); BoxFrame.Size = UDim2.new(1,0,0,32); BoxFrame.BackgroundColor3 = Library.Theme.Main; Instance.new("UICorner", BoxFrame).CornerRadius = UDim.new(0, 4); Instance.new("UIStroke", BoxFrame).Color = Library.Theme.Outline
				...
				Input.TextSize = Library.Theme.TextSize.Element -- [DYNAMIC]
