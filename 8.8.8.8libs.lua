...
function Library:CreateWelcomeScreen(customText)
	...
	Label.Text = customText or "8.8.8.8 <font color='#AA00FF'>VIRTUAL</font>"
	Label.RichText = true
	Label.TextColor3 = Color3.new(1, 1, 1)
	Label.Font = Enum.Font.GothamBold
	Label.TextSize = 0 -- Départ à 0
	Label.TextTransparency = 1
	
	-- Animation d'entrée (Surgit et devient gros)
	TS:Create(Label, TweenInfo.new(1, Enum.EasingStyle.Back), {TextSize = 80, TextTransparency = 0}):Play()
	
	task.delay(2.5, function()
		TS:Create(Label, TweenInfo.new(1, Enum.EasingStyle.Quart), {TextSize = 120, TextTransparency = 1}):Play()
		Debris:AddItem(Screen, 1.2)
	end)
end
...
function Library:CreateWindow(title)
	...
	Main.Position = UDim2.new(0.5, -325, 0.5, 0) -- Start lower for anim
	...
	-- Animation d'ouverture du menu (Glissement vers le haut)
	Main.GroupTransparency = 1
	TS:Create(Main, TweenInfo.new(0.6, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = UDim2.new(0.5, -325, 0.5, -225)}):Play()
	...
	TabContainer.Size = UDim2.new(1, -10, 1, -80); TabContainer.Position = UDim2.new(0, 5, 0, 70); TabContainer.BackgroundTransparency = 1; TabContainer.ScrollBarThickness = 0
	...
	function WindowActions:AddPage(name)
		...
		TabBtn.Size = UDim2.new(1, 0, 0, 35); TabBtn.BackgroundTransparency = 1; TabBtn.Text = "      " .. name; TabBtn.TextColor3 = Theme.TextDark; TabBtn.Font = "GothamMedium"; TabBtn.TextSize = 13; TabBtn.TextXAlignment = "Left"; TabBtn.ClipsDescendants = true
		Instance.new("UICorner", TabBtn).CornerRadius = UDim.new(0, 6)
		
		local SelectionBar = Instance.new("Frame", TabBtn)
		SelectionBar.Size = UDim2.new(0, 3, 0, 0); SelectionBar.Position = UDim2.new(0, 8, 0.5, 0); SelectionBar.BackgroundColor3 = Theme.Accent; SelectionBar.BorderSizePixel = 0
		
		TabBtn.MouseButton1Click:Connect(function()
			for _, p in pairs(PageContainer:GetChildren()) do p.Visible = false end
			for _, b in pairs(TabContainer:GetChildren()) do if b:IsA("TextButton") then 
				TS:Create(b, TweenInfo.new(0.3), {BackgroundTransparency = 1, TextColor3 = Theme.TextDark}):Play()
				TS:Create(b.Frame, TweenInfo.new(0.3), {Size = UDim2.new(0,3,0,0), Position = UDim2.new(0,8,0.5,0)}):Play()
			end end
			Page.Visible = true
			TS:Create(TabBtn, TweenInfo.new(0.3), {BackgroundTransparency = 0.9, TextColor3 = Theme.Text}):Play()
			TS:Create(SelectionBar, TweenInfo.new(0.3), {Size = UDim2.new(0,3,0,18), Position = UDim2.new(0,8,0.5,-9)}):Play()
			CreateRipple(TabBtn)
		end)

		if first then 
			Page.Visible = true; TabBtn.BackgroundTransparency = 0.9; TabBtn.TextColor3 = Theme.Text
			SelectionBar.Size = UDim2.new(0,3,0,18); SelectionBar.Position = UDim2.new(0,8,0.5,-9)
			first = false 
		end
		...
		function PageActions:AddSection(sTitle)
			...
			function SecActions:AddButton(t, c)
				local B = Instance.new("TextButton", Sec); B.Size = UDim2.new(0.95, 0, 0, 32); B.BackgroundColor3 = Theme.Main; B.Text = "  " .. t; B.TextColor3 = Theme.Text; B.Font = "Gotham"; B.TextSize = 13; B.TextXAlignment = "Left"; B.AutoButtonColor = false; B.ClipsDescendants = true
				Instance.new("UICorner", B).CornerRadius = UDim.new(0, 4); local S = Instance.new("UIStroke", B); S.Color = Theme.Outline
